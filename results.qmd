...
-   Western states tend to have lower rates cross all delay types, and security delay rate is the smallest, which is shown as distint cluster around 0.
...
state_flow <- delay_airport %>%
  ...
  drop_na(region) %>%       # <-- DROP US TERRITORIES / NA REGIONS
...
plot_data <- left_join(
  ...
  mutate(
    ...
    region = state.region[match(state_abb, state.abb)]  # <- add this
  ) |>
  filter(!is.na(region))  # <- drop DC / anything without a region
